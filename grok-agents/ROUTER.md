...
- Two agents surface the same event: the one whose core mission owns it keeps it, the other drops it and notes the overlap in `killed`. Ownership: model behavior is Sweep Radar, technique is Drift Scout, publishing claim is Receipts Desk, discourse-timing is Reply Window, program is Scope Watch, ticker sentiment is Earnings Static.
